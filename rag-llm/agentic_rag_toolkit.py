...
规则：
- 单次范围不超过20个chunk（end_chunk_index - start_chunk_index + 1 ≤ 20），超出请分多次调用
...
B. 如果未知 file_name：
   - 若有部分文件名线索（如用户提到文件名关键词、或已知文件命名规律），使用 list_filename_by_like 先找候选文件
...
- retrieved_round：用于判断该 chunk 是哪轮检索到的，辅助判断检索进展
