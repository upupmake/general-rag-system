...
from prompts import CLAUDE_DISGUISE_SYSTEM_PROMPT, append_runtime_system_context
...
async def process_rag_stream_events(stream_iterator, prompt_tokens: int = 0):
    ...
    async for item in stream_iterator:
        if item["type"] == "process":
            ...
            yield f"data: {json.dumps(data, ensure_ascii=False)}\n\n"
        elif item["type"] == "provider_response":
            cached_tokens = item["payload"].get("cachedTokens")
            data = {
    ...
    if cached_tokens is not None:
        usage_data["payload"]["cached_tokens"] = cached_tokens
...
async def stream_generator(model_instance, messages, prompt_tokens: int = 0, options: dict = None):
    ...
    if messages and messages[0].get("role") == "system":
        messages[0]["content"] = append_runtime_system_context(messages[0]["content"])
    else:
        messages.insert(0, {
            "role": "system",
            "content": append_runtime_system_context()
        })

    async for item in unified_llm_stream(model_instance, messages):
        content = item["payload"]
        if item["type"] == "provider_response":
            cached_tokens = content.get("cachedTokens")
            data = {
                "type": "provider_response",
                "payload": content
            }
...
def build_langchain_messages(history: list) -> list:
    ...
    for msg in history:
        ...
        if role == 'user':
            ...
            provider_response_items = msg.get('providerResponseItems')
            if provider_response_items:
                additional_kwargs['providerResponseItems'] = provider_response_items
...
async def chat_stream(
    ...
    if use_rag and kb_id and user_id:
        max_rounds = options.get('maxRounds', 10)
    ...
    # 否则使用纯LLM模式
        ...
        llm = get_official_llm(
            ...
            prompt_cache_key=options.get('promptCacheKey'),
        )
        # 添加当前问题到消息列表
        ...
        if options.get("_claude_disguise"):
            system_message = CLAUDE_DISGUISE_SYSTEM_PROMPT + (f"\n\n{system_message}" if system_message else "")
        if system_message:
