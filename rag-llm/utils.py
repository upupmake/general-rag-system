...
async def image_split(
    ...
    # 4. 构造 Prompt
    prompt = (
        "请先判断图片类型（如纯文本/信息图/数据图表/截图/照片等），然后按需输出以下字段（无内容则跳过）：\n"
        "【文字】逐行转录图中所有清晰可辨的文字，保持原有顺序、换行和标点\n"
        "【布局】描述图片的构图方式、排版结构、视觉层次、色彩风格或设计特点\n"
        "【图表】若含数据图表/表格，说明类型、坐标轴含义、数据趋势、关键数值或对比关系\n"
        "【关键词】提取图中的技术术语、专有名词、缩写、核心概念等，用逗号分隔\n"
        "【摘要】用1句话客观概括图片传达的核心信息或主要意图\n"
        " 输出格式：仅输出有内容的字段，每行格式严格遵循：【字段名】+ 内容\n"
        " 不添加编号、额外说明、markdown标记或解释性文字。"
    )
    messages = [
        {
            "role": "user",
