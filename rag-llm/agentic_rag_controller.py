...
- 已知 documentId 需要连续阅读时，使用 read_file_chunks
- 需要探索性检索或缺乏明确关键词时，使用 semantic_search
- 已找到关键chunk需要上下文时，使用 expand_context

### 2. 充分利用已检索信息
- 关注文档的 documentId、fileName、chunkIndex、maxChunkIndex
- 避免重复检索相同内容
- 已定位关键 chunk 且只需上下文时，优先 expand_context

### 3. 已检索结果去重机制
- 系统会记录已经进入参考文档的 chunk，并在 keyword_search 和 semantic_search 中尽量排除这些已检索 chunk。
- 因此，重复使用完全相同的 keyword_search 或 semantic_search 参数时，返回结果可能是同一检索方向下尚未看过的后续 chunk，而不是同一批结果。
- 如果相同参数重复检索多轮后仍有新增，应判断是否确实需要继续穷举；若只是为了回答当前问题，应优先换查询角度、使用 read_file_chunks/expand_context 精读关键位置，或调用 stop_search。
- read_file_chunks 和 expand_context 使用 documentId 保持同一文档的连续上下文，可能查到已读 chunk；系统会在最终参考文档中按 chunk 去重。
...
适用场景：用户问题涉及特定文件
```
find_files → 获取 documentId → keyword_search 或 read_file_chunks
```
...
### read_file_chunks
- document_id 必须来自检索结果中的 documentId
- 参考 maxChunkIndex 避免越界
- 单次不超过20个chunk

### expand_context
- document_id 和 chunk_index 必须来自已命中的chunk
