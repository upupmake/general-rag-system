...
通过 General RAG MCP，你可以让支持 MCP 的 Agent 在获得授权后创建私有或公开知识库、检索知识库、阅读原文，并管理本人创建的个人私有知识库文件。
...
<div class="doc-fact-grid">
  <div><strong>9</strong><span>个原子工具</span></div>
...
> MCP 目前只能创建**私有**或**公开**知识库，不支持创建工作空间共享知识库。文件上传和删除只允许操作**当前 Access Key 用户本人创建的个人私有知识库**。
...
<figure class="doc-shot">
  ...
  <figcaption><strong>图 8 · 打开 MCP 配置面板</strong><span>后台显示成功连接并加载 9 个工具，表示服务地址和认证信息有效。</span></figcaption>
</figure>
...
<div class="doc-callout success">
  ...
  <span>Agent 后台显示成功连接 MCP 服务 <code>kb</code>，并提示已加载 9 个工具。</span>
</div>
...
| `create_knowledge_base` | 创建当前 Access Key 用户拥有的知识库 | 创建私有或公开知识库；暂不支持工作空间共享 |
...
你可以直接告诉 Agent 创建一个私有或公开知识库：

```text
请创建一个名为“产品公开文档”的公开知识库，
描述为“面向客户的产品使用说明”。创建前先向我确认名称和可见性。
```

创建时需要注意：

- `name` 必填，最多 100 个字符；
- `description` 可选，最多 200 个字符；
- `visibility` 只能是 `private` 或 `public`，默认是 `private`；
- 新知识库归当前 Access Key 对应的用户所有，不会关联工作空间；
- 工作空间共享知识库暂时仍需在网页端创建。
...
MCP 创建工具目前只接受 `private` 或 `public`。如需创建工作空间共享知识库，请登录网页端，在目标工作空间中创建。
...
5. 在执行创建知识库、上传、删除等会修改数据的操作前，要求 Agent 先展示目标并等待确认。
6. 工具调用会记录必要的审计信息，但不会记录完整 Access Key、检索到的 Chunk 正文、知识库描述正文或上传文件内容。
